...
int main(int argc, char ** argv) 
{
    ...
    {
        baza->lista_przedmiotow = sortuj_przed_przez_wybrany_kod(baza->lista_przedmiotow); 
        listuj_przedmioty_od_konca(baza);
    } 
    
    else if (strcmp("sort_course_name", argv[2]) == 0) 
    {
        baza->lista_przedmiotow = sortuj_przed_przez_wybrany_nazwa(baza->lista_przedmiotow); 
        listuj_przedmioty_od_konca(baza);
    } 
    
    else if (strcmp("filter_courses", argv[2]) == 0) 
    {
        baza->lista_ocen = filtruj_przedmioty(baza->lista_ocen, argv[3], baza);
        listuj_oceny(baza);
    } 
    
    else if (strcmp("filter_courses2", argv[2]) == 0) 
    {
        baza->lista_ocen = filtruj_przedmioty2(baza->lista_ocen, argv[3], baza);
        listuj_oceny(baza);
    }
